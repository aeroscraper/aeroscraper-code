...
export async function fetchUserStakeState(
  ...
  // Parse UserStakeAmount struct (from IDL lines 3555-3580)
  ...
  // Parse p_factor from state (skip: discriminator + admin + oracle_helper + oracle_state + fee_distributor + fee_state + 2 bytes + stable_coin + total_debt + total_stake)
  ...
  stateOffset += 2; // minimum_collateral_ratio + protocol_fee
  ...
  // For precision, multiply first then divide
  const compounded_stake = (amount * p_factor) / p_snapshot;
